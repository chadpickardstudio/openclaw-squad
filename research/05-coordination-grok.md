...
|-------------------------|----------------------------------------------------|-----------------------------------------------|
| Flexibility             | High (dynamic decomposition)                       | Low (fixed sequence)                          |
| Debugging/Audit         | Excellent (files + bindings CLI)                   | Poor (nested session history)                 |
| Recursion Risk          | Low (depth limits + skips + denies)                | High                                          |
| Scalability (5–19 agents) | Excellent                                          | Poor (token/cost explosion)                   |
| Cost Optimization       | Native (model & tool tiering per role)             | Difficult                                     |
| Inspectability          | High (append-only logs)                            | Low                                           |
| Failure Recovery        | Easy (restart Lead, re-read plan.md)               | Brittle                                       |

**Pros of loose**: auditable, resilient, aligns with OpenClaw's isolation model.
**Cons**: requires disciplined Lead prompt engineering.
**Pros of rigid**: predictable for tiny squads.
**Cons**: over-engineering; community reports abandonment after 3–4 agents.
...
```json
{
  "name": "sessions_send",
  "parameters": {
    "sessionKey": "agent:coder:telegram:group:123",  // or sessionId from sessions_list
    "message": "Implement feature X per plan.md line 42. Return concise diff.",
    "timeoutSeconds": 0   // 0 = fire-and-forget; >0 = wait for reply
  }
}
```
...
**shenhao-stu 9-agent squad** (github.com/shenhao-stu/openclaw-agents – one-command deploy):
Agents: main (final), planner (CEO/Lead), ideator, critic (SHARP scoring gate), surveyor, coder, writer, reviewer, scout.
...
**LumaDock-style 19-agent fleet** (tutorial patterns + YouTube reports):
One Coordinator Lead + 18 stateless domain specialists (research variants, UX, security, analytics, social, etc.).
...
**Pantheon-style 14-agent marketing squad** (Reddit r/openclaw + community reports, 2026):
Lead + specialists (content, SEO, ad-copy, analytics, email, social variants).
...
### Integration with Independence & Memory
...
Specialists often run with `memory.enabled: false` (stateless). Lead never receives full MEMORY.md dumps; reads summaries via announce or read-only file access if shared dir configured. Oversight only.

### Integration with Cost Optimization

- Lead/coordinator: cheap model (Claude-Sonnet-mini, Gemini-flash, local) for decomposition/routing
...
- Lead "grants" tools at setup time via config (no runtime grant)

### Common Failure Modes and Anti-Patterns

- Hard-coded chains = over-engineering → brittle on changes
- Broadcast messaging (`sessions_send` to many) = noise & cost explosion
...
# 3. Enable opt-in A2A & depth limits
...
**Binding priority rule**: Place most-specific bindings first in the array (though most-specific match always wins).
...
This pattern set (bindings + controlled sessions_send + file handoffs + Lead-CEO) is the production standard in March 2026. It delivers reliable, auditable, cost-efficient squads far beyond early 2025 rigid pipelines. For custom 14+ agent configs or Pantheon-style marketing templates, clone shenhao-stu and layer LumaDock governance prompts.