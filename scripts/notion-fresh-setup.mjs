#!/usr/bin/env node
/**
 * IntegrateAI — Notion Workspace Cleanup & Fresh Setup
 *
 * 1. Finds and archives ALL existing IntegrateAI databases and pages
 * 2. Creates one clean IntegrateAI HQ page
 * 3. Creates the 3 databases with proper organization
 *
 * Usage:
 *   NOTION_API_KEY=ntn_xxx node scripts/notion-fresh-setup.mjs
 */
...
// ---------------------------------------------------------------------------
// Step 1: Delete everything related to IntegrateAI
// ---------------------------------------------------------------------------

async function cleanAll() {
  console.log("━━━ STEP 1: Cleaning up old items ━━━\n");

  // Search for ALL items we have access to
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    for (const item of search.results) {
      ...
      const isOurs =
        title.includes("IntegrateAI") ||
        title.includes("CRM") ||
        title.includes("Launch Checklist") ||
        title.includes("Decision Log");

      if (isOurs) {
        try {
          if (item.object === "database") {
            await notion("PATCH", `/databases/${item.id}`, { archived: true });
          } else {
            await notion("PATCH", `/pages/${item.id}`, { archived: true });
          }
          console.log(`  🗑  Archived ${item.object}: "${title}"`);
          archived++;
        } catch (err) {
          console.log(`  ⚠  Could not archive "${title}": ${err.message}`);
        }
      }
    }

    hasMore = search.has_more;
    startCursor = search.next_cursor;
  }

  console.log(`\n  Archived ${archived} items total.\n`);
}
...
// ---------------------------------------------------------------------------
// Step 2: Create fresh IntegrateAI HQ page
// ---------------------------------------------------------------------------

async function createHQPage() {
  console.log("━━━ STEP 2: Creating IntegrateAI HQ ━━━\n");
  ...
  // Find a top-level page (one whose parent is the workspace)
  let parentPage = search.results.find(
    (p) => !p.archived && p.parent?.type === "workspace"
  );

  if (!parentPage) {
    // Use any non-archived page
    parentPage = search.results.find((p) => !p.archived);
  }

  if (!parentPage) {
    console.error("✗ No accessible pages found. Share a page with your integration.");
    process.exit(1);
  }

  console.log(`  Using parent: ${getTitle(parentPage)} (${parentPage.url})`);

  // Create IntegrateAI HQ as a child page
  const hq = await notion("POST", "/pages", {
    ...
    children: [
      {
        ...
        callout: {
          icon: { type: "emoji", emoji: "👋" },
          rich_text: rt(
            "Welcome to IntegrateAI Advisors HQ. This is your central hub for CRM, launch tracking, and decision logging."
          ),
        },
      },
      {
      ...
      },
      {
        object: "block",
        type: "heading_2",
        heading_2: { rich_text: rt("🎯 Client Pipeline (CRM)") },
      },
      {
        object: "block",
        type: "paragraph",
        paragraph: {
          rich_text: rt("Track leads from first contact through to closed deals."),
        },
      },
      {
        object: "block",
      ...
      },
      {
        object: "block",
        type: "heading_2",
        heading_2: { rich_text: rt("🚀 Launch Checklist") },
      },
      {
        object: "block",
        type: "paragraph",
        paragraph: {
          rich_text: rt("4-week launch plan with 59 tasks across all categories."),
        },
      },
      {
        object: "block",
      ...
      },
      {
        object: "block",
        type: "heading_2",
        heading_2: { rich_text: rt("📋 Decision Log") },
      },
      {
        object: "block",
        type: "paragraph",
        paragraph: {
          rich_text: rt(
            "Record of all strategic decisions with rationale and ownership."
          ),
        },
      },
    ],
  ...
  console.log(`  ✓ Created IntegrateAI HQ: ${hq.url}\n`);
  return hq.id;
}
...
async function createCRM(parentId) {
  console.log("━━━ STEP 3a: Creating CRM ━━━\n");

  const db = await notion("POST", "/databases", {
    parent: { type: "page_id", page_id: parentId },
    icon: { type: "emoji", emoji: "🎯" },
    title: rt("CRM — Client Pipeline"),
    properties: {
      Name: { title: {} },
      Status: {
        select: {
...
async function createLaunchChecklist(parentId) {
  console.log("━━━ STEP 3b: Creating Launch Checklist ━━━\n");

  const db = await notion("POST", "/databases", {
    parent: { type: "page_id", page_id: parentId },
    icon: { type: "emoji", emoji: "🚀" },
    title: rt("Launch Checklist"),
    properties: {
      Task: { title: {} },
      Status: {
        select: {
...
async function createDecisionLog(parentId) {
  console.log("━━━ STEP 3c: Creating Decision Log ━━━\n");

  const db = await notion("POST", "/databases", {
    parent: { type: "page_id", page_id: parentId },
    icon: { type: "emoji", emoji: "📋" },
    title: rt("Decision Log"),
    properties: {
      Decision: { title: {} },
      Date: { date: {} },
      Rationale: { rich_text: {} },
  ...
  for (const d of decisions) {
    await notion("POST", "/pages", {
      parent: { database_id: db.id },
      properties: {
        Decision: { title: rt(d.decision) },
        Date: { date: { start: d.date } },
        Rationale: { rich_text: rt(d.rationale) },
        "Decided By": { select: { name: d.decidedBy } },
        Category: { select: { name: d.category } },
        "Reference Doc": { rich_text: rt(d.ref) },
      },
    });
  }
...
async function main() {
  console.log("╔══════════════════════════════════════════════╗");
  console.log("║  IntegrateAI — Fresh Notion Workspace Setup  ║");
  console.log("╚══════════════════════════════════════════════╝\n");

  // Step 1: Clean everything
  await cleanAll();

  // Step 2: Create HQ page
  const hqId = await createHQPage();

  // Step 3: Create databases
  const crmId = await createCRM(hqId);
  const checklistId = await createLaunchChecklist(hqId);
  const decisionLogId = await createDecisionLog(hqId);

  console.log("╔══════════════════════════════════════════════╗");
  console.log("║  ✓ DONE — Your workspace is ready!           ║");
  console.log("╚══════════════════════════════════════════════╝");
  console.log(`\n  IntegrateAI HQ page ID: ${hqId}`);
  console.log(`  CRM DB:            ${crmId}`);
